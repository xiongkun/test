...
public class RemoveServer {
          ...
          */
                  // =====================================
          public static int getPenalty(String line, int pos) {
...
//            }
//            return r;
              String[] statuses = line.split("\\s+");
              int penalty = 0;
              for(int i = 0; i < statuses.length; i++){
                  int status = Integer.parseInt(statuses[i]);
                  if((status == 1 && i < pos) || (status == 0 && i>= pos)){
                      penalty++;
                  }
              }
              return penalty;
...
//            return r;
              String[] statuses = line.split("\\s+");
              int[] leftDown = new int[statuses.length];
              int[] rightUp = new int[statuses.length];
              ...
              for(int i = 0; i < statuses.length; i++){
                  leftDown[i] = downs;
                  if("1".equals(statuses[i]))
                      downs++;
                  int penalty = leftDown[i]+rightUp[i];
                  if(penalty < min){
                      min = penalty;
                      minPos  = i;
                  }
...
//            return r;
              ...
              for(String token : tokens){
                  if("BEGIN".equals(token)){
                      open = true;
                  }else if(open && "END".equals(token)){
                      String input = validInput.toString().trim();
                      if(!input.isEmpty()) {
                          bestTimes.add(bestTimeToShutDown(input));
                      }
                      validInput = new StringBuilder();
                      open = false;
                  }else if(open && ("1".equals(token) || "0".equals(token))){
                      validInput.append(token).append(" ");
                  }
              }
              return bestTimes;
          }
...
//            System.out.println(getPenalty("0 0 1 0", 4) == 1);
            ...
            System.out.println(bestTimeToShutDown("1 0 0") == 0); // 0 or 2
            System.out.println(Arrays.toString(parseLogs("BEGIN END BEGIN BEGIN 0 0 1 0 END END 0 0 1").toArray())
                                                            );
            System.out.println(Arrays.toString(parseLogs("BEGIN BEGIN 1 0 0 END").toArray()).equals("[0]"));
            System.out.println(Arrays.toString(parseLogs("BEGIN BEGIN 1 0 0 END 0 0 0 1 BEGIN 1 1 1 0 END").toArray())
                                                            .equals("[0, 0]"));
          }
}
