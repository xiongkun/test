 ...
 */
public class HttpHeaderLangResolver {
...
//      return r;
//    }
    HashSet<String> langDict;
    HashMap<String, Set<String>> langTable;
    public HttpHeaderLangResolver(Set<String> supported){
        //part 1
        langDict = new HashSet<>(supported);
        //part 2
        langTable = new HashMap<>();
        for(String langStr : supported){
            //TODO: validate
            langStr = langStr.trim();
            String[] items = langStr.split("-");
            String lang = items[0].trim().toLowerCase();
            langTable.putIfAbsent(lang, new HashSet<>());
            langTable.get(lang).add(langStr.trim());
        }
    }

    public List<String> getAcceptedLang1(String required){
        String[] langs = required.split(",");
        //TODO: validate input and handle exceptions
        List<String> answers = new ArrayList<>();
        for(String lang : langs){
            if(langDict.contains(lang.trim())){
                answers.add(lang.trim());
            }
        }
        return answers;
    }
    ...
    public List<String> getAcceptedLang2(String required){
        String[] langStrs = required.split(",");
        List<String> matches = new ArrayList<>();
        for(String langStr : langStrs){
            langStr = langStr.trim();
            String[] items = langStr.split("-");
            String lang = items[0].trim().toLowerCase();
            if(langDict.contains(langStr)){
                matches.add(langStr);
            } else if(langTable.containsKey(lang)){
                Set<String> langSet = new HashSet<>(langTable.get(lang));
                langSet.removeAll(matches);
                matches.addAll(langSet);
            }
        }
        return matches;
    }
    ...
    public List<String> getAcceptedLang3(String required){
        String[] langStrs = required.split(",");
        List<String> matches = new ArrayList<>();
        for(String langStr : langStrs){
            langStr = langStr.trim();
            ...
            if(langDict.contains(langStr)){
                ...
                langSet.removeAll(matches);
                matches.addAll(langSet);
            }else if("*".equals(langStr)){
                Set<String> langSet = new HashSet<>(langDict);
                langSet.removeAll(matches);
                matches.addAll(langSet);
            }
        }
